...
export type QuotaDialogProps = {
  context: Context
  data: QuotaDashboardData
  selected: number
...
export function QuotaDialog(props: QuotaDialogProps): JSX.Element {
  ...
  const tabs = total > 1 ? (
    <box flexDirection="row" gap={1}>
      ...
      {data.providers.map((item, itemIndex) => Math.abs(itemIndex - selected) <= 1 ? (
        <box onMouseDown={() => props.onSelect(itemIndex)}>
          <text fg={selected === itemIndex ? theme.text.base : theme.text.muted} attributes={selected === itemIndex ? 1 : 0}>
            {selected === itemIndex ? `[ ${tabName(item, data.providers)} ]` : `  ${tabName(item, data.providers)}  `}
          </text>
        </box>
      ) : null)}
      {data.errors.length && Math.abs(data.providers.length - selected) <= 1 ? (
        <box onMouseDown={() => props.onSelect(data.providers.length)}>
